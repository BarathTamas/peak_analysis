...
def plot_processing(
    ...
    baseline_fitter: BaseLineFitter,
    peak_detector: PeakDetector,
    peak_detector_comp: PeakDetector,
    s_stdev: pd.Series,
):
    ...
    df_proc, df_peaks = process_data(
        ...
        peak_detector=peak_detector_comp,
        s_stdev=s_stdev
    )
    earliest_peak_idx: int = peak_detector.earliest_peak_idx
    ...
    for cell_nr in cell_nr_list:
        ...
        min_peak_amplitude: float = peak_detector.min_peak_amplitude
        if cell_nr in df_peaks.index:
            ...
            _color_list = [
                "red" if x >= min_peak_amplitude else "green" for x in _peak_amplitudes
            ]
            # axes[1].plot(
